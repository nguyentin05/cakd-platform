...
## TASK
Write a quickstart tutorial that takes a user from zero to a running cloud-native project in under 10 minutes. Base all steps strictly on the provided Go source code — only document commands, flags, and environment variables that actually exist.
...
```bash
{installation commands based on task/source files}
```
...
:::tip Expected output
{describe what a successful run looks like based on the create/pipeline stdout messages}
:::
...
{describe what a successful startup of cakd-agent looks like based on main.go}
:::

## Step 5: Verify the result

```bash
{verification commands, e.g. cakd validate}
```

## What was created

{list of what got generated, based on scaffold mappings and terraform output}
